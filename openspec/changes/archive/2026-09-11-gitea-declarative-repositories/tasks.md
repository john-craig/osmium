...
- [x] 1.1 Define repository option schema for stable local keys, user or
...
- [x] 1.2 Add repository persistence and runtime credential references without
...
- [x] 1.3 Add repository reconciliation after owner identities are available;
...
- [x] 2.1 Extend normalized Gitea drift observation to include user-owned and
  ...
  result
- [x] 2.2 Compare declared repository attributes with observed values and emit
...
- [x] 2.3 Convert repository drift into review-only Mythoclast declarations;
...
- [x] 2.4 Add completeness, provenance, ambiguity, content-scope, and sensitive
...
- [x] 3.1 Implement live repository capture from the running Gitea API with
  ...
  incomplete findings
- [x] 3.2 Convert live capture output into the same deterministic Mythoclast
...
- [x] 4.2 In that MicroVM mutate repository metadata externally, run the live
  drift detector, and verify the drift artifact records the observed change and
  preserves repository content and ownership
- [x] 4.3 Convert the runtime-generated drift into a candidate, feed that exact
...
- [x] 4.4 Verify the drift path's secret omission, incomplete/unsupported state,
...
- [x] 5.1 Add separate flake check
  ...
  organization
- [x] 5.2 Capture the running service and verify the artifact contains the
  runtime-created repositories, correct owner references, supported metadata,
  provenance, and deterministic ordering
- [x] 5.3 Convert the runtime-generated capture artifact and feed that exact
  ...
  fixture
- [x] 5.4 Verify the live-capture path's secret omission, incomplete/unsupported
...
- [x] 6.1 Document repository declaration fields, owner dependencies, supported
...
- [x] 6.3 Run `openspec validate gitea-declarative-repositories --strict` and
  verify the change passes strict validation
- [x] 6.4 Run `nix flake check --no-build --no-update-lock-file` and verify all
  outputs evaluate
- [x] 6.5 Execute both dedicated MicroVM checks and report their results
