...
Live capture SHALL reconstruct every supported profile rule, skill reference,
MCP reference, model, enabled state, and permission attribute from runtime facts.
It MUST be deterministic, review-only, secret-free, explicit about unresolved
host or credential inputs, and MUST NOT claim completeness for unavailable or
