...
- [x] 1.1 Define the versioned remote capture artifact schema for provenance,
...
- [x] 1.2 Define canonical serialization, stable identity keys, origin markers,
...
- [x] 1.3 Define the adapter/probe interface and allowlisted probe vocabulary,
...
- [x] 2.1 Implement SSH transport configuration with host-key verification,
...
- [x] 2.2 Implement the standard NixOS adapter to discover the Gitea service
...
- [x] 2.3 Implement read-only Gitea observation through the API and/or local
...
- [x] 2.4 Record probe provenance, missing optional probes, unsupported fields,
...
- [x] 3.1 Convert a complete capture into deterministic Mythoclast Gitea user,
...
- [x] 3.2 Convert safe secret references without reading secret bytes and emit
...
- [x] 3.3 Classify administrators, unsafe names, ambiguous owners, external
...
- [x] 3.4 Preserve capture and conversion as separate non-mutating commands and
...
- [x] 4.1 Document and implement adapter capability/version discovery so future
  Linux adapters emit the same capture schema and explicitly report omissions
- [x] 4.2 Add a synthetic adapter fixture and contract tests proving transport,
...
- [x] 5.3 Verify generated output is deterministic, secret-free, contains
...
- [x] 6.1 Document SSH trust and privilege requirements, supported NixOS layouts,
  capture scope, artifact review, secret handling, completeness, conversion,
  unsupported state, and activation boundaries
- [x] 6.2 Run `nix fmt` and verify no formatting changes remain
- [x] 6.3 Run `openspec validate remote-gitea-capture-to-declarative-config --strict`
  and verify the change passes strict validation
- [x] 6.4 Run `nix flake check --no-build --no-update-lock-file` and verify all
  outputs evaluate
- [x] 6.5 Run the dedicated remote capture integration check and report the
